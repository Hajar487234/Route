...
// Seleziona l'icona e aggiungi l'evento
const shovelIcon = document.getElementById('shovel');
shovelIcon.addEventListener('click', () => {
    showGrid = !showGrid; // Inverte lo stato (on/off)
   
    // Opzionale: aggiungi un feedback visivo sull'icona
...
function animate() {
    ...
    // Disegno griglia (solo se showGrid è true)
    if (showGrid) {
        c.save();
        c.translate(Math.floor(-halfMapW - camX), Math.floor(-halfMapH - camY));
